...
function onAssetsLoaded(loader, resources) {
  ...
  function startPlay() {
    ...
    for (let i = 0; i < reels.length; i++) {
      if (!reels[i]) {
        console.log('nema reel-a', reels[i])
        return
      }
      const r = reels[i]
      const extra = Math.floor(Math.random() * 3)
      const target = Math.round(r.position + 10 + i * 15 + extra)
      ...
      for (let j = 0; j < reels[i].symbols.length; j++) {
        ...
        if (j == 2) {
          let reelSym = reels[i].symbols[
            num
          ]._texture.textureCacheIds.toString()
          state.reelSymbols[i] = reelSym.substring(7, reelSym.length - 4)
        }
      }
